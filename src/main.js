...
import { Shop, applyTheme } from './screens/shop.js';
import { PlayerState } from './state/playerState.js';
import { initFirebaseUser, syncToCloud } from './services/firebaseSetup.js';
...
Router.add('#/miner-map', (r) => import('./screens/minerMap.js').then(m => m.MinerMap(r)));
...
App.addListener('backButton', () => {
  ...
  const gameModes = ['#/classic', '#/hex', '#/sort', '#/2048', '#/x2', '#/merge', '#/duel', '#/adventure', '#/match', '#/bubble', '#/miner-map'];
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  // Init Firebase and wait for auth
  initFirebaseUser().then((user) => {
    if (user) {
      ...
      MultiplayerService.listenForChallenges((challengeData) => {
        ...
        banner.innerHTML = `
          <div class="flex items-center gap-3 w-full">
            <div class="relative shrink-0">
              <img src="/avatars/${challengeData.challengerAvatar}.png" onerror="this.src='/avatars/akita.png'" class="w-12 h-12 rounded-full border-2 border-cyan-400 shadow-md" />
        ...
        import('./utils/sounds.js').then(m => m.Sounds.playSfx('success'));
        import('./utils/haptics.js').then(m => m.Haptics.vibrate('heavy'));
        ...
        let responded = false;
          ...
          PlayerState.save();
          import('./components/toast.js').then(m => m.Toast.show('İstek reddedildi. Ana menüye dönene kadar bildirim almayacaksınız.', 'info'));
          closeBanner();
        };
      ...
      import('./services/friendService.js').then(({ FriendService }) => {
        ...
        FriendService.listenToFriendRequests((requests) => {
          PlayerState.state.friendRequests = requests;
          PlayerState.notify();
        });

        // Pre-fetch friends list on load
        FriendService.getFriendsList().then(list => {
          PlayerState.state.friends = list;
          PlayerState.notify();
        });
      });
    }
